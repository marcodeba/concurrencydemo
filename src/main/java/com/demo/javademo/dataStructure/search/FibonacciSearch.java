...
public class FibonacciSearch {
    ...
    public static int fibonacciSearch(int key, int[] a) {
        ...
        System.out.println("斐波那契角标 [" + (index - 2) + "] 位为：" + getFibonacci(index - 2));
        System.out.println("斐波那契角标 [" + (index - 1) + "] 位为：" + getFibonacci(index - 1));
        System.out.println();
        ...
        //初始化记录首位 末位 mid非字面意义上的中间值，仅是将数组分割为两部分
        int low = 0;
        int high = a.length - 1;
        int mid;
        while (low <= high) {
            //计算分割数组处的角标
            mid = low + fibonacciArray.get(index - 1) - 1;
            System.out.println("斐波那契角标 =  " + index + "   将key与数组的角标 [" + (mid) + "] 所在的值做比较");
            if (key < temp[mid]) {
                high = mid - 1;
                index -= 1;
                ...
                //如果值相等且角标小于或等于待查数组的最大角标 返回mid 表示找到 
                if (mid <= a.length - 1) {
                    return mid;
                } else {
                    //如果值相等但角标大于待查数组的最大角标 
